...
bool CTxDB::LoadBlockIndex()
{
    ...
    loop
    {
        ...
        if (strType == "blockindex")
        {
            ...
            // Watch for genesis block and best block 如果是创世区块则保存到pindexGenesisBlock
    ...
    pindexBest = mapBlockIndex[hashBestChain]; // 最长链索引
    nBestHeight = pindexBest->nHeight; // 最长链高度
...
bool CAddrDB::LoadAddresses()
{
    CRITICAL_BLOCK(cs_mapAddresses)
    {
        ...
        if (filein)
        {
            try
            {
                ...
                while (fgets(psz, sizeof(psz), filein))
                {
                    ...
                    if (addr.ip != 0)
                        AddAddress(*this, addr); // 将mapAddresses更新到数据库
                }
            }
            catch (...) { }
        }
        ...
        loop
        {
            ...
            CDataStream ssKey;
            CDataStream ssValue;
            // 将游标pcursor处数据序列化读取给ssKey、ssValue
...
// CWalletDB
//
// CWalletDB在构造函数中会自动加载wallet.dat
bool CWalletDB::LoadWallet(vector<unsigned char>& vchDefaultKeyRet)
{
    ...
    CRITICAL_BLOCK(cs_mapWallet)
    {
        ...
        loop
        {
            ...
            string strType;
            ssKey >> strType; // strType的类型决定ssValue的数据反序列化给哪个对象
            if (strType == "name")
            {
                string strAddress;
                ssKey >> strAddress;
...
bool LoadWallet()
{
    ...
    if (mapKeys.count(vchDefaultKey)) // 若mapKeys中含有vchDefaultKey
    {
        // Set keyUser
        // 将vchDefaultKey中的公私钥对填入keyUser
