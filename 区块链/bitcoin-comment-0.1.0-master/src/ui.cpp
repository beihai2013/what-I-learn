...
void CSendDialog::OnButtonSend(wxCommandEvent& event)
{
    ...
    // AddressToHash160 得到收款人公钥哈希，且创建脚本
    uint160 hash160;
    ...
    if (fBitcoinAddress)
    {
        ...
        if (!SendMoney(scriptPubKey, nValue, wtx)) //真正的交易创建函数
            return;
...
bool CMyApp::OnInit()
{
    try
    {
        return OnInit2(); // 命令行参数初始化，对argc、argv进行分割
    }
...
bool CMyApp::OnInit2()
{
    ...
    //
    // Limit to single instance per user 限制用户单例
    // Required to protect the database files if we're going to keep deleting log.* 
    // char *getenv(const char *name)搜索name指向的环境字符串并返回
    // 获取比特币钱包的安装路径
    wxString strMutexName = wxString("Bitcoin.") + getenv("HOMEPATH");
    for (int i = 0; i < strMutexName.size(); i++)
        if (!isalnum(strMutexName[i])) // 检查字符是否为数字或字母
            strMutexName[i] = '.';
    // wxSingleInstanceChecker保证参数代表的程序在执行中是单例模式
    wxSingleInstanceChecker* psingleinstancechecker = new wxSingleInstanceChecker(strMutexName);
    // IsAnotherRunning 若程序已经在运行，返回true；否则返回false
    if (psingleinstancechecker->IsAnotherRunning()) 
    {
        ...
        loop
        {
            ...
            // IpClassName为指向一个指定了类名的空结束字符串，或一个标识类名字符串的成员的指针
            // IpWindowName指向一个指定了窗口名（窗口标题）的空结束字符串
            // 以下均为UI界面窗口的操作，略过
            HWND hwndPrev = FindWindow(L"wxWindowClassNR", L"Bitcoin");
            if (hwndPrev)
            {
                if (IsIconic(hwndPrev)) // 判断窗口是否最小化
    ...
    {
        nDropMessagesTest = atoi(mapArgs["/dropmessages"]);// atoi将字符串转换为整型
    ...
    if (!LoadAddresses())   // 加载地址
        strErrors += "Error loading addr.dat      \n";
    QueryPerformanceCounter((LARGE_INTEGER*)&nEnd); 
    printf(" addresses   %20I64d\n", nEnd - nStart); // %20I64d 控制只输出20位

    printf("Loading block index...\n");
    QueryPerformanceCounter((LARGE_INTEGER*)&nStart);
    if (!LoadBlockIndex()) // 加载块索引
    ...
    //
    {
        ...
        if (fGenerateBitcoins) //节点进行挖矿
            if (_beginthread(ThreadBitcoinMiner, 0, NULL) == -1) // 开启矿工挖矿线程
                printf("Error: _beginthread(ThreadBitcoinMiner) failed\n");

        //
        // Tests
        // 解析命令
        if (argc >= 2 && stricmp(argv[1], "/send") == 0) // 等于0表示相等
        {
            int64 nValue = 1;
            if (argc >= 3)
