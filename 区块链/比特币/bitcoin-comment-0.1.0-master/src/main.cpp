...
bool AddToWallet(const CWalletTx& wtxIn)
{
    ...
    CRITICAL_BLOCK(cs_mapWallet)
    {
        ...
        // hash是交易的hash值，wtxIn是交易
        // insert原型：pair<iterator,bool>insert(const value_type& x);
        ...
        if (!fInsertedNew)
        {
            ...
            // Merge
            bool fUpdated = false;
            // hashBlock代表交易所在block的hash值，从默克尔树继承来
            if (wtxIn.hashBlock != 0 && wtxIn.hashBlock != wtx.hashBlock)
            {
                wtx.hashBlock = wtxIn.hashBlock;
                fUpdated = true;
            }
            // vMerkleBranch代表交易所在的Merkle树分支
            // nIndex代表当前交易在对应的block对应的输入vtx列表中的索引
...
bool AddToWalletIfMine(const CTransaction& tx, const CBlock* pblock)
{
    ...
    // map.count 返回被查找元素的个数
    // tx.isMine 判断当前交易是否属于本节点
...
int64 CWalletTx::GetTxTime() const
{
    if (!fTimeReceivedIsTxTime && hashBlock != 0)
    {
        ...
        if (mi != mapBlockIndex.end())
        {
            ...
            if (pindex)
                return pindex->GetMedianTime(); // 获取当前对应块下标的中位数时间
        }
    }
...
bool CTransaction::RemoveFromMemoryPool()
{
    ...
    CRITICAL_BLOCK(cs_mapTransactions)
    {
        ...
        ma pTransactions.erase(GetHash());
        nTransactionsUpdated++;
    }
    return true;
}
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast)
{
    ...
    bnNew /= nTargetTimespan; // bnNew *= nActualTimespan/nTargetTimespan，即得到与之前难度的倍数
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    // Check for duplicate
    uint256 hash = GetHash();
    if (mapBlockIndex.count(hash)) // 检查是否重复
    ...
    if (pindexNew->nHeight > nBestHeight)
    {
        ...
        else if (hashPrevBlock == hashBestChain) // 若为最长链，直接添加当前区块
        {
			// 如果当前块对应的前一个块是最长的链
            // Adding to current best branch
            if (!ConnectBlock(txdb, pindexNew) || !txdb.WriteHashBestChain(hash)) // 无法连接或者数据库无法写入
            {
                txdb.TxnAbort();
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // 前一区块是否存在
    if (!mapBlockIndex.count(pblock->hashPrevBlock)) // 不存在，加入到孤儿区块表中
    {
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    loop
    {
        ...
        CMessageHeader hdr;
        // 将vRecv反序列化到hdr中
        // 读取数据后，读取指针会指向数据域的开始，下次再开始时直接读取数据域
        vRecv >> hdr; // 指针已经偏移了
        if (!hdr.IsValid())
        {
        ...
        if (nMessageSize > vRecv.size())
        {
            ...
            vRecv.insert(vRecv.begin(), BEGIN(hdr), END(hdr)); // 插入消息头
            Sleep(100);
            break;
        }
        ...
        vRecv.ignore(nMessageSize); // vRecv指针向后偏移nMessageSize长度
...
bool ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        static bool fAskedForBlocks; // 静态局部标记，保证了只对第一个连接到的节点发送getblocks消息
    ...
    // 要求节点在处理version之外的任何信息时，version不为0
    ...
    else if (strCommand == "inv")
    {
        ...
        foreach(const CInv& inv, vInv)
        {
            ...
            bool fAlreadyHave = AlreadyHave(txdb, inv); // 本地节点是否知道这个库存消息
    ...
    else if (strCommand == "getdata")
    {
        ...
        foreach(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK) // 代表请求的是具体数据
            {
            ...
            else if (inv.IsKnownType()) // 如果消息是已知的其他类型，转播消息
            {
    ...
    {
        CBlockLocator locator; // 初始块
        uint256 hashStop; // 结尾块
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            ...
            CRITICAL_BLOCK(pfrom->cs_inventory)
            {
				...
				// 判断在已知库存2中是否存在
                // returns true if wasn't already contained in the set 当inv不在库存2中时，返回true
    ...
    else if (strCommand == "tx")
    {
        ...
        CInv inv(MSG_TX, tx.GetHash()); // MSG_TX库存存储交易的hash
        ...
        {
            AddToWalletIfMine(tx, NULL); // 将交易加入钱包
            RelayMessage(inv, vMsg);// 转播交易的库存消息
        ...
        else if (fMissingInputs) //缺少输入的交易，判断为孤儿交易，放入孤儿交易池
        {
    ...
    // 对自己之前发送消息的确认。若找到确认消息的hash，则擦除该消息请求。
...
bool SendMessages(CNode* pto)
{
    ...
    CRITICAL_BLOCK(cs_main)
    {
        ...
        // Message: addr
        // 定义一个临时的待发送地址池
        // 遍历节点的待发送地址中的地址，判断这些地址是不是在该节点的已知地址集合中
        // 是则将该地址放入临时定义的待发送地址池vAddrToSend。然后清空节点的待发送地址池。
        // 如果临时的待发送地址池非空，则向该节点推送addr消息。
        ...
        // 库存消息处理
        // Message: inventory
        // 定义一个临时的待发送库存池，与上面类似，遍历该节点的待发送库存池
        // 如果这条库存不是该节点的已知库存，则将库存推送到临时待发送库存池。
        // 之后清空节点的待发送库存池和已知库存2池。这之后如果临时带发送库存池不是空的，则需要向节点推送库存inv消息。
...
bool BitcoinMiner()
{
    ...
    {
        Sleep(50);
        CheckForShutdown(3); // 设置线程标志，vfThreadRunning【3】为false
        while (vNodes.empty())
        {
            Sleep(1000);
            CheckForShutdown(3);
        ...
        // 构建交易输入
        txNew.vin.resize(1);
        txNew.vin[0].prevout.SetNull(); // 前一个交易的输出为null
        txNew.vin[0].scriptSig << nBits << ++bnExtraNonce; // 输入脚本的签名设置为nBit挖矿难度+bnExtraNonce随机数
        // 构建交易输出
        txNew.vout.resize(1); // 何用？？
        txNew.vout[0].scriptPubKey << key.GetPubKey() << OP_CHECKSIG; //交易对应的公钥设置为key的公钥+OP_CHECKSIG
...
{
    setCoinsRet.clear(); // 需要花费掉的输入 ，即UTXO
    ...
    CRITICAL_BLOCK(cs_mapWallet)
    {
        for (map<uint256, CWalletTx>::iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (!pcoin->IsFinal() || pcoin->fSpent)
                continue;
            int64 n = pcoin->GetCredit(); // 如果不是当前用户的钱包，则返回0
            if (n <= 0)
                continue;
            if (n < nTargetValue)
            {
                vValue.push_back(make_pair(n, pcoin)); // 钱不够，放入vValue中
                nTotalLower += n;   // 所有不够的输入的总和
            }
            else if (n == nTargetValue)
            {
                setCoinsRet.insert(pcoin); // 钱刚好能花完，放入setCoinsRet中等待消费
                return true;
            }
            else if (n < nLowestLarger) // 此时n>nTagetValue，找一个满足n>nTargetValue的最小值
            {
                nLowestLarger = n;
    ...
    if (nTotalLower < nTargetValue) // 所有比所需比特币数小的输入，加起来都不能满足所需的比特币数
    {
        if (pcoinLowestLarger == NULL)  // 没有满足的输入
            return false;
        setCoinsRet.insert(pcoinLowestLarger); // 找一个满足n>nTargetValue的最小值，将其放入setCointsRet
        return true;
    }
    // 此时情况是，所有比所需比特币数小的输入加起来满足所需比特币数
    // 此时优先花费所有比所需比特币数小的输入
    ...
    for (int nRep = 0; nRep < 1000 && nBest != nTargetValue; nRep++)
    {
        ...
        for (int nPass = 0; nPass < 2 && !fReachedTarget; nPass++) // nPass只有0和1两种取值
        {
            for (int i = 0; i < vValue.size(); i++)
            {
                // 第一次执行，即nPass为0时，随机选择执行不执行if中的语句
                // 第二次执行，即nPass为1时，如果在第一次执行中没有选择花费该输入，则执行if中的语句
                if (nPass == 0 ? rand() % 2 : !vfIncluded[i]) 
                {
                    ...
                    {
                        fReachedTarget = true; // 花销的所有比目标nTargetValue小的输入，已经超过nTargetValue
                        if (nTotal < nBest)
                        {
                            nBest = nTotal;
                            vfBest = vfIncluded;
                        ...
                        nTotal -= vValue[i].first;
                        vfIncluded[i] = false; // 目标值已达到，不需要在花销输入UXTO
                    }
                }
            }
        }
...
bool CreateTransaction(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, int64& nFeeRequiredRet)
{
    ...
    CRITICAL_BLOCK(cs_main)
    {
        ...
        CRITICAL_BLOCK(cs_mapWallet)
        {
            int64 nFee = nTransactionFee; // 交易费
            loop
            {
                wtxNew.vin.clear();
                wtxNew.vout.clear();
                ...
                if (!SelectCoins(nValue, setCoins)) // 选择合适的UTXO，即选择合适的输入，放入setCoins
                    return false;
                int64 nValueIn = 0;
...
bool SendMoney(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew)
{
    ...
    {
        int64 nFeeRequired;
        if (!CreateTransaction(scriptPubKey, nValue, wtxNew, nFeeRequired)) // 创建交易 111
        {
            string strError;
        ...
        if (!CommitTransactionSpent(wtxNew)) // 提交交易
        {
        ...
        // Broadcast
        if (!wtxNew.AcceptTransaction()) // 接收交易
        {
        ...
        wtxNew.RelayWalletTransaction(); //广播交易
    }
    MainFrameRepaint();
    return true;
}
