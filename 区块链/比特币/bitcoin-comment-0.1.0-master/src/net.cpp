...
void ThreadSocketHandler2(void* parg)
{
    ...
    loop
    {
        ...
        CRITICAL_BLOCK(cs_vNodes)
        {
            ...
            // 断开vNodes中的重复节点
            // 对相同ip的连接节点，释放较小的引用，添加较大的引用
            // vNode进行了默认排序，后遍历到的节点较大
            map<unsigned int, CNode*> mapFirst;
            foreach(CNode* pnode, vNodes)
            {
                if (pnode->fDisconnect) //标记节点是否已经断开连接
                    continue;
            ...
            // 节点处于准备释放状态（引用为0，fDisconnect为true，发送缓冲区和接收缓冲区为空）
            // 节点从节点列表删除，断开连接，放入节点断开池
		...
		*/
        // Select函数对fdsetRecv监听套接字接收消息池、fdsetSend监听套接字发送消息池进行设置
        ...
        // 接受新的连接并保存到vNodes中
        if (FD_ISSET(hListenSocket, &fdsetRecv)) // 查看hListenSocket是否被置位；若被置位则代表有其他节点的连接请求
        {
            struct sockaddr_in sockaddr;
            int len = sizeof(sockaddr);
            SOCKET hSocket = accept(hListenSocket, (struct sockaddr*)&sockaddr, &len); // 接收socket链接
            CAddress addr(sockaddr);
            if (hSocket == INVALID_SOCKET) // 非法socket
            {
        ...
        foreach(CNode* pnode, vNodesCopy)
        {
            ...
            if (FD_ISSET(hSocket, &fdsetRecv))
            {
                TRY_CRITICAL_BLOCK(pnode->cs_vRecv)
                {
                    CDataStream& vRecv = pnode->vRecv; // vRecv为接收缓冲区
...
void ThreadOpenConnections2(void* parg)
{
    ...
    {
        // Wait
        vfThreadRunning[1] = false; // 节点链接线程标记
        Sleep(500);
        // vNode为全局变量，为与本地节点相连的节点
        // 实际节点数大于最大连接数，则需要暂停1秒
        ...
        vfThreadRunning[1] = true;
        CheckForShutdown(1); // 检查全局变量fShutdown是否为true。若为true，则关闭线程
        ...
        CRITICAL_BLOCK(cs_mapAddresses) // CRITICAL_BLOCK包含的部分为互斥访问字段，用来使互斥数据不被多线程同时访问
        {
        ...
        while (!fSuccess && nLimit-- > 0)
        {
            ...
            CRITICAL_BLOCK(cs_mapAddresses)
            {
				...
				*/
                // 遍历mapAddress中所有网络为ipC的地址。此处地址包含端口号
                // ipC格式为xxx.xxx.xxx.000，为当前C类子网下最小ip
                     ...
                     ++mi)
                {
                    ...
                    unsigned int nRandomizer = (addr.nLastFailed * addr.ip * 7777U) % 20000; //何用？？？
					// 当前时间 - 地址连接最新失败的时间 要大于对应节点重连的间隔时间
                    if (GetTime() - addr.nLastFailed > nDelay * nRandomizer / 10000) // 操作意义不明？？？
            ...
            foreach(const CAddress& addrConnect, (*mi).second)
            {
                ...
                if (addrConnect.ip == addrLocalHost.ip || !addrConnect.IsIPv4() || FindNode(addrConnect.ip))
                    continue;
				// 链接对应地址信息的节点
                // ConnectNode函数封装了connect函数，会返回一个填写好的节点信息
                ...
                // 从新连接的节点，发送getaddr命令，获取更多的节点地址信息
                ...
                fSuccess = true; // 已经建立了连接，结合之前的while判断条件，可以退出
                break;
            }
        }
    }
...
// 消息处理线程
void ThreadMessageHandler2(void* parg) // prag代表主线程创建的监听线程，是经过listen函数处理的被动套接字
{
    ...
    loop
    {
        ...
        foreach(CNode* pnode, vNodesCopy)
        {
            ...
            // Receive messages
            TRY_CRITICAL_BLOCK(pnode->cs_vRecv)
                ProcessMessages(pnode); // 处理节点接收到的消息

            // Send messages
            TRY_CRITICAL_BLOCK(pnode->cs_vSend)
                SendMessages(pnode); // 处理节点发送的消息
...
bool StartNode(string& strError)
{
    ...
    // Sockets startup
    WSADATA wsadata;
    // socket编程用于初始化winsockDLL
    // 第一个参数为wVersionRequested，调用windows socket的最高版本；高字节指定小版本号，低字节指定高版本号
    // 第二个参数为指向WSADATA数据结构体指针
    // 若成功，函数返回0；否则返回对应的错误码
	...
	*/
    ...
    // 启动IRC地址线程，处理其他节点连接请求
    // IRC类似于一个聊天室，频道内的人能够收到消息，频道外的人不能
